...
var Table = _react2['default'].createClass({
  ...
  getThead: function getThead() {
    ...
    var columns = ths.map(function (c, index) {
      var colSpan = c.colSpan;
      var sort = "";
      if (colSpan !== 0) {
        if (_this.props.expandIconAsCell && !_this.props.expandIconColumnHeader && index === 0) {
          // if expand icon is rendered as icon and expandIconColumnHeader is false, we need to span second column header
          colSpan = colSpan || 1;
          colSpan += 1;
        }
        if (_this.props.sortColumn === c.dataIndex) {
          sort = _this.props.orderBy === 0 ? "asc" : "desc";
        }
    ...
    return _react2['default'].createElement(
      'thead',
      { className: this.props.prefixCls + '-thead', onClick: this.onHeadCellClick },
  ...
  getRowsByData: function getRowsByData(data, visible, indent) {
    ...
    var childrenColumnName = _props.childrenColumnName;
    var expandedRowRender = _props.expandedRowRender;
    var expandIconAsCell = _props.expandIconAsCell;
    var expandOnRowClick = _props.expandOnRowClick;
    var rowKey = _props.rowKey;
    var rowClassName = _props.rowClassName;
    var expandedRowClassName = _props.expandedRowClassName;
    ...
    for (var i = 0; i < data.length; i++) {
      var record = data[i];
      var key = rowKey ? rowKey(record, i) : undefined;
